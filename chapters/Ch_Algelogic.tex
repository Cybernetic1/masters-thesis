...
\newcommand{\bigMagentaArrow}[0]{{\LARGE{\color{magenta}\RIGHTarrow}}}
\begin{enumerate}
	\item The \bigMagentaArrow indicates \textbf{internal data} supplied by the Transformer, similar to the Q,K,V matrices in traditional Transformers.  These are modified through ``learning'' or training.
...
\subsubsection{Overall explanation of the diagram}
Start from recalling that logic rules are stored as a rectangular matrix of parameters:

These parameters are passed as \bigMagentaArrow from the left of the diagram.  When I say ``select $J$ out of $K$ predicates,''  it means applying Softmax which is explicitly shown in the diagram.  Softmax does not need other data parameters -- it is just an operation that exaggerates the differences among the input components.  
