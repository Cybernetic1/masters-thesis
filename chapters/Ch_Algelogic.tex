...
\begin{equation}
	\logic{father}(\vcenter{\hbox{\includegraphics[scale=0.5]{logic-variables1.png}}})
	\wedge
	\logic{father}(\vcenter{\hbox{\includegraphics[scale=0.5]{logic-variables2.png}}})
	\rightarrow
	\logic{grand\hbox{-}father}(\vcenter{\hbox{\includegraphics[scale=0.5]{logic-variables3.png}}})
\end{equation}
...
\textbf{Legend:}
\begin{enumerate}
	\item The magenta {\LARGE\color{magenta}\RIGHTarrow} indicates \textbf{internal data} supplied by the Transformer, similar to the Q,K,V matrices in traditional Transformers.  These are modified through ``learning'' or training.
	\item The 3 black \RIGHTarrow \ are the \textbf{inputs} to the Transformer, this is matched with the 3 \textbf{outputs} on the right of the diagram.
	\item Orange tags {\color{orange}$[M,N,...]$} indicate the \textbf{dimensions} of data streams.
\end{enumerate}
...
The operation $a \stackrel{?}{=} b$ can be implemented by this function:
\begin{equation}
