...
Analyzing a more-detailed example:
\begin{equation}
...
\end{equation}
where the blue bars represent fuzzy-values, some kind of relative proportion of variables, such that the overall construction would be differentiable.
...
\subsubsection{How to deal with variables in rules}

( Besides $(p, \gamma)$ we seem to need another parameter, the variable number specifying which of $\{ v_1, v_2, ... v_n \}$ is referenced in a rule's argument.  This seems to be a parameter independent of $(p, \gamma)$.  But it seems irrelevant if there is no cylindrification? )
