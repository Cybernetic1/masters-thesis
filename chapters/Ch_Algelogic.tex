...
A better solution is to associate the weight to that argument of the predicate such that the resulting \textbf{true value} of that predicate would be \textbf{weakened}.  The truth value of a predicate $P$ is evaluated by sending a point $(a,b)$ as input to the neural network representing $P$.  Its output is a real number normalized $\in [ 0,1 ]$ and interpreted as a fuzzy truth-value.
