...
\begin{enumerate}  % [label=\protect\circled{\arabic*}]
	\item[\textbullet] The \textbf{grayed-out} parts of the circuit tries to match the \textbf{predicates} of Working-Memory propositions to those in the rules.  For example, the predicate $\mathsf{loves}$ where $\mathsf{loves(John,Mary)}$ matches $\mathsf{loves}(X_1,X_2)$.  But I take the more liberal approach where matching succeeds whenever a point in Working Memory falls within the target predicate's region of definition, regardless of whether the source and target predicates match or not.  For example, assume $\mathsf{human(Socrates)}$ is in Working Memory, and we want to match against $\mathsf{mortal(X_1)}$.  This will succeed if the $\mathsf{human}$ region lies strictly within the $\mathsf{mortal}$ region in Subject Space.  If the predicate regions are learned properly, this would be the case.  Using this approach, rules are more easily satisfied, and the code seems to be simpler.
